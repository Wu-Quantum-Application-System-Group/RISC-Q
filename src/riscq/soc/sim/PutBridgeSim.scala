...
import riscq.soc.link.{Put, PutBridge, PutLink}
import riscq.soc.spec.SocSpecMap

import scala.collection.mutable.ArrayBuffer

/**
 * Sign-off for [[PutBridge]] + the far-side demux: drive the bridge's TileLink slave with a
 ...
 *
 * Run with `./.metals/mill runMain riscq.soc.sim.PutBridgeSim`.
 */
object PutBridgeSim extends App {
  val putAddrWidth = SocSpecMap.putAddrWidth   // the put window: nodes 0..15 = own channels, ≥ 16 = system
  ...
  case class Dut() extends Component {
    val tlBus = new MasterBus(tilelink.M2sParameters(
      addressWidth = putAddrWidth, dataWidth = 32,
    ...
    val gate = master port Flow(Put(16)); gate << PutLink.demux(bridge.cmd, gateBase, winSize, 16)
    val ro   = master port Flow(Put(16)); ro   << PutLink.demux(bridge.cmd, roBase,   winSize, 16)
    val sys  = master port cloneOf(bridge.cmd);          sys  << PutLink.nonLocal(bridge.cmd, SocSpecMap.localNodes)
  }
  ...
  SimConfig.compile(Dut()).doSim("putBridge", seed = 42) { dut =>
    ...
    // ── ordered posted stream matches the issued writes (absolute put-window address + data) ──
    val expectedCmd = issued.map { case (a, d) => (BigInt(a), BigInt(d) & 0xFFFFFFFFL) }
    assert(cmdBeats == expectedCmd,
      s"posted Put stream mismatch:\n  got  $cmdBeats\n  want $expectedCmd")
    ...
    println(s"[PutBridgeSim] PASS  putAddrWidth=$putAddrWidth: ${issued.size} TL stores → ${cmdBeats.size} ordered " +
      s"Put beats (gate ${gateBeats.size}, readout ${roBeats.size}, system ${sysBeats.size}); all acked locally (no stall).")
    simSuccess()
  }
}
